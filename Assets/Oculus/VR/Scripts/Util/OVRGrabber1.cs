...
public class OVRGrabber1 : MonoBehaviour
{
    ...
    [SerializeField]
    protected OVRInput.Controller m_controller = OVRInput.Controller.LTouch;
    ...
    //여러개 잡도록 하려고 선언함.
    protected float m_prev2Flex;
    protected bool m_prevButton;
    ...
    void OnUpdatedAnchors()
    {
        ...
        //검지와 중지 모두 상용하기 위하여 하나 더 선언함. hand트리거는 중지로 누르는것
        m_prev2Flex = OVRInput.Get(OVRInput.Axis1D.PrimaryHandTrigger, m_controller);

        //m_prevButton = OVRInput.Get(OVRInput.Button.PrimaryHandTrigger);
    ...
    {
        //이부분에서 잡는 부분을 설정해 줄 수 있다.
        //조건문은 중지를 눌렀을 때로 처리.
        if (m_prev2Flex >= grabBegin)
        {
            //중지를 누른 이후 검지로 누르면 잡을 수 있게 만든다.
            if ((m_prevFlex >= grabBegin) && (prevFlex < grabBegin))
            {
                GrabBegin();
            }
            // 중지가 눌린상태에서 검지를 풀면 잡은걸 던진다.
    ...
    protected virtual void GrabBegin()
    {
        ...
        if (closestGrabbable != null)
        {
            ...
            {
                closestGrabbable.grabbedBy1.OffhandGrabbed1(closestGrabbable);
            }

            m_grabbedObj = closestGrabbable;
            m_grabbedObj.GrabBegin1(this, closestGrabbableCollider);
    ...
    protected virtual void OffhandGrabbed1(OVRGrabbable grabbable)
    {
